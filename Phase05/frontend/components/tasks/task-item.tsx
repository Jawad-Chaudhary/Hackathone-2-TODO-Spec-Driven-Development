...
// [Task T037] Phase 5: Display priority, tags, due date, and recurrence indicators
// [Task T095] Add Framer Motion animations to task list items
...
export const TaskItem: React.FC<TaskItemProps> = ({ task, userId, onDelete, onUpdate }) => {
  ...
  const getRecurrenceText = () => {
    ...
    if (task.recurrence === "custom" && task.recurrence_interval) {
      return `Every ${task.recurrence_interval} day${task.recurrence_interval > 1 ? "s" : ""}`;
    }
  ...
  return (
    ...
    >
      <Card variant="default" className={isOverdue ? "border-l-4 border-l-red-500" : ""}>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* [Task T037] Phase 5 Metadata: Priority, Tags, Due Date, Recurrence */}
                  ...
                  className={`inline-flex items-center px-2 py-1 rounded text-xs ${
                    new Date(task.due_date) < new Date() && !isCompleted
                      ...
                      : new Date(task.due_date) <
                        new Date(Date.now() + 24 * 60 * 60 * 1000) && !isCompleted
